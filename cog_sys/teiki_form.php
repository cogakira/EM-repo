...
function get_teiki_form($id) {
    ...
    $sql = 'SELECT * FROM teiki WHERE id = :id;';
    $pre = $dbh->prepare($sql);
 ...
 *
 */
function validate_teiki_form($datum) {
    // エラー情報の詳細を入れるための配列を用意する
    $error_detail = [];

    // 必須チェックを実装
    $validate_params = ['mcode','hjunkyocd', 'rackno','shohincd','keikucd'];
foreach($validate_params as $p) {
    ...
    */
if($datum['role'] > 1 ){
    if (1 !== preg_match('/\A[0-9]{3}[- ]?[0-9]{4}\z/', $datum['yubin'])) {
...
function format_yubin($yubin) {
    return str_replace(array('-', ' '), array('', ''), $yubin);
}
