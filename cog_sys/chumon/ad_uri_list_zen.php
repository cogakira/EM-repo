...
// $shukko = 0;
$stmt = $dbh->prepare('select sum(seikyu) as seikyu from uriage where shukko = :shukko ;');
$stmt->bindValue(':shukko',$shukko,PDO_PARAM_INT);
$stmt->execute();
if ($row = $stmt->fetch()) {
    $kei = $row['seikyu'];
}
...
$sql = 'SELECT * FROM uriage where shukko = 0 order by chumonbi desc,koshinbi desc;';
// ここ
$pre = $dbh->prepare($sql);
