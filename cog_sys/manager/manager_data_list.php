...
function a_tag_print($type, $out) {
  if ($type === $GLOBALS['sort']) {
      echo "<a class='bg-danger text-danger' href='./manager_data_list.php?sort={$type}'>{$out}</a>";
  } else {
      echo "<a class='text-muted' href='./manager_data_list.php?sort={$type}'>{$out}</a>";
  }
}
...
<body>
<h1 style='text-align:center;border:solid 2px;width:400px;margin:0 auto;'>会員・モニター一覧</h1>
...
<div class="row">
  <form action="./manager_data_list.php" method="post">
    <div>
  ...
  <?php if (false === empty($search)) : ?>
    ...
    ?>
    <br>
    <a class="btn btn-default" href="./manager_data_list.php">検索項目をクリアする</a>
<?php endif;?>
...
<tr>
  ...
  <td width="30"><a class="btn btn-default" href="./manager_data_detail.php?id=<?php echo rawurlencode($datum['id']); ?>">詳細</a>
  <td width="30"><a class="btn btn-default" href="./manager_data_update.php?id=<?php echo rawurlencode($datum['id']); ?>">修正</a>
  <td width="30"><form action="./manager_data_delete.php" method="post">
...
<div class="row">
    <?php if (1 != $page_num): ?>
      <a class="btn btn-default" href="./manager_data_list.php?p=<?php echo ($page_num - 1); ?>&sort=<?php echo rawurlencode($sort); ?>">＜＜前</a>
    <?php endif; ?>

    <?php if ($max_page_num > $page_num): ?>
      <a class="btn btn-default" href="./manager_data_list.php?p=<?php echo ($page_num + 1); ?>&sort=<?php echo rawurlencode($sort); ?>">次＞＞</a>
    <?php endif; ?>
  </div>
  ...
  <div class="row">
    ...
    <?php for($i = 1; $i <= $max_page_num; ++$i): ?>
        ...
        <?php else: ?>
            <li><a href="./manager_data_list.php?p=<?php echo $i; ?>&sort=<?php echo rawurlencode($sort); ?>"><?php echo $i; ?></a></li>
        <?php endif; ?>
    <?php endfor; ?>
    </ul>
  </div>
