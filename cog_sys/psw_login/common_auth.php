...
function login($input_password, $datum, $lock_table) {
    // 判定用フラグ
    $login_flg = false;
    // DBハンドルの取得
    $dbh = get_dbh();
    ...
    if (false === empty($datum)) {
        ...
        $sql = 'SELECT * FROM ' . $lock_table . ' WHERE id=:id;';
        $pre = $dbh->prepare($sql);
        // 値のバインド
        $pre->bindValue(':id', $datum['id'], PDO::PARAM_STR);
        // SQLの実行
        $r = $pre->execute(); // XXX
        // SELECTした内容の取得
        ...
        if ('0000-00-00 00:00:00' === $lock_datum['lock_time']) {
            ...
            if (true === password_verify($input_password, $datum['password'])) {
                // countのリセット
                $lock_datum['error_count'] = 0;
                // ログインフラグを立てる
                $login_flg = true;
        ...
        $sql = 'REPLACE INTO ' . $lock_table . '(id, error_count, lock_time) VALUES(:id, :error_count, :lock_time);';
        $pre = $dbh->prepare($sql);
        // 値のバインド
        $pre->bindValue(':id', $lock_datum['id'], PDO::PARAM_STR);
