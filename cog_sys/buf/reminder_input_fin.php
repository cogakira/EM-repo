...
//var_dump($token);
    ...
    $sql = 'INSERT INTO reminder_token(token,id, created) VALUES(:token, :id, :created);';
    $pre = $dbh->prepare($sql);
    // 値のバインド
    $pre->bindValue(':token', $token, PDO::PARAM_STR);
    $pre->bindValue(':id', $datum['id'], PDO::PARAM_INT);
...
http://localhost:8888/cog_sys/buf/reminder_password_input.php?t={$token}
EOD;
