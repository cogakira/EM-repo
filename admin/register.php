...
<?php if ( (isset($view_data['error_overlap_id']))&&(true === $view_data['error_overlap_id']) ) : ?>
...
<?php endif; ?>
  ...
  <tr>
    <th>ログイン用ID
    <td><input name="id" value="<?php echo h(@$view_data['id']); ?>">
  <tr>
    <th>名前
    <td><input name="name" value="<?php echo h(@$view_data['mname']); ?>">
  <tr>
    <th>役割
    <td>
      <label><input name="role" type="radio" value="0" <?php if (0 == @$view_data['role']): ?>checked<?php endif; ?> >閲覧のみ</label>
      <label><input name="role" type="radio" value="1" <?php if (1 == @$view_data['role']): ?>checked<?php endif; ?> >仮会員通常操作まで</label>
      <label><input name="role" type="radio" value="2" <?php if (2 == @$view_data['role']): ?>checked<?php endif; ?> >本会員</label>
      <label><input name="role" type="radio" value="3" <?php if (3 == @$view_data['role']): ?>checked<?php endif; ?> >モニター本会員管理</label>
      <label><input name="role" type="radio" value="4" <?php if (4 == @$view_data['role']): ?>checked<?php endif; ?> >全て管理できる</label>
  <tr>
    <th>パスワード
