...
// 「パラメタの一覧」を把握
$params = array('id', 'mname', 'role', 'pass_1', 'pass_2');
// データを取得する
foreach($params as $p) {
...
$sql = 'INSERT INTO mdata(id, name, password, role, created) VALUES(:id, :mname, :password, :role, :created);';
$pre = $dbh->prepare($sql);

// 値のバインド
$pre->bindValue(':id', $user_input_data['id'], PDO::PARAM_STR);
...
$pre->bindValue(':password', user_pass_hash($user_input_data['pass_1']), PDO::PARAM_STR);
...
//var_dump($e);
    if (0 === strncmp($e[2], 'Duplicate entry', strlen('Duplicate entry'))) {
        // エラー情報をセッションに入れて持ちまわる
        $_SESSION['output_buffer']['error_overlap_id'] = true;
        // 入力値も持ちまわる
        $_SESSION['output_buffer'] += $user_input_data;
        // 入力ページに遷移する
        header('Location: ./register.php');
